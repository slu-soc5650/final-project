...
```{block, type = "rmdgoal"}
**Goal:** Obtain additional geometric data to use for mapping.
```
...
**Skills:** This vignette requires skills from both Module 2 (especially Meeting 5) and Module 3 (especially Meeting 6)
```

```{block, type = "rmddue"}
**Required Due Date 1:** Meeting 08 (March 22nd) as part of [Waypoint 4](index.html#waypoints)

**Required Due Date 2:** Final Submission (May 17th)
```

```{block, type = "rmddeliver"}
**Deliverables:** A well organized `data/raw/` folder with your original data sources, all of which should be re-projected, cleaned, and stored in `data/`. There should be a corresponding `.Rmd` file for any data downloaded via the `tigris` package as well as data cleaning and re-projection.
```
